...
export async function createTag(userId: string, tagName: string, sessionToken: string) {
...
export async function updateTag(tagId: string, newName: string, sessionToken: string) {
  try {
...
export async function getNotesByTag(tagId: string, sessionToken: string) {
  try {
    const response = await fetch(`http://localhost:8000/api/v1/notes/filter_by_tag?tag_id=${tagId}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "session-token": sessionToken
    ...
    if (!response.ok) {
      console.warn(`No se encontraron notas para el tag ${tagId}:`, await response.text());
      return [];
    }

    const notesForTag = await response.json();
    return notesForTag.map((noteWrapper: { note: { id: any; title: any; content: any; }; }) => ({
